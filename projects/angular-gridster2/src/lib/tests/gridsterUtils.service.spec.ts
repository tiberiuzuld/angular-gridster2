...
describe('merge method', () => {
    it('should check if merge is working correctly', () => {
        ...
        expect(JSON.stringify(GridsterUtils.merge(obj1, obj2, properties))).not.toBe(
...
describe('check content class', () => {
    ...
    it('should check in classList', () => {
        expect(GridsterUtils.checkContentClass(event.target, event.currentTarget, 'divClass')).toBe(
            true,
        );

        expect(GridsterUtils.checkContentClass(event.target, event.currentTarget, 'body')).toBe(
            true,
        );
    });
});
